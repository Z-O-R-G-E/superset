...
const REGEX = {
  ...
  INTERVAL: /^(\d+\s+)?\d{1,2}:\d{1,2}:\d{1,2}(\.\d+)?$/,
    ...
    switch (typeUpper) {
      ...
      case 'INTERVAL': {
        if (!REGEX.INTERVAL.test(stringValue)) {
          return error('Формат интервала: [DD ]HH:MM:SS[.дробные_секунды]');
        }

        const [daysPart, timePart] = stringValue.split(/\s+/);
        let hours;
        let minutes;
        let seconds;
        ...
        if (timePart) {
          ...
          [hours, minutes, seconds] = daysPart.split(':').map(Number);
        }

        if (hours >= 24) return error('Часы должны быть меньше 24');
        if (minutes >= 60) return error('Минуты должны быть меньше 60');
        if (seconds >= 60) return error('Секунды должны быть меньше 60');
